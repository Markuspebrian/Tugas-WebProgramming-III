            ...
            <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                <div class="navbar-nav">
                    ...
                    if (!empty($this->session->userdata('email'))) { ?>
                        <a class="nav-item nav-link mr-2" href="<?=base_url('booking')?>">Booking Buku
                        <b><?= $this->ModelBooking->getDataWhere('temp',['email_user'=> $this->session->userdata('email')])->num_rows();?></b>Buku</a>
                        <a class="nav-item nav-link mr-2" href="<?= base_url('member/myProfil'); ?>">Profil Saya</a>
                        <a class="nav-item nav-link mr-2" href="<?= base_url('autentifikasi/logout'); ?>"><i class="fas fw fa-login"></i> Log out</a>
                    <?php } else { ?>
                        <a class="nav-item nav-link mr-2" data-toggle="modal" data-target="#daftarModal" href="<?= base_url('Autentifikasi/registrasi'); ?>"><i class="fa-solid fa-id-card"></i> Daftar</a>
                        <a class="nav-item nav-link mr-2" data-toggle="modal" data-target="#loginModal" href="<?= base_url('Autentifikasi'); ?>"><i class="fa-solid fa-right-to-bracket"></i> Log in</a>
                    <?php } ?>
